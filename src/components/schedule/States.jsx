  ...
  const getLabelForWebcast = (webcast, index, total) => {
    ...
    const hasDivisions = eventData && (
      ...
      (eventData.name && (
        ...
        (eventData.name.toLowerCase().includes('state') || eventData.name.toLowerCase().includes('district'))
      ))
    );
       
    if (hasDivisions) {
      // For state events with divisions, use division numbers
        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10 reveal-bottom">
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">
              <span className="bg-gradient-to-r from-[hsl(275,60%,20%)] to-[hsl(275,60%,80%)] bg-clip-text text-transparent">
                {eventData ? eventData.name : "Michigan State Championship"}
              </span>
            </h2>
          </div>
          ...
          {loading ? (
              ...
              <div className="grid md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                  ...
                  {eventData.webcasts && eventData.webcasts.length > 0 ? (
                    ...
                    <p className="text-gray-400">Webcasts will be available during the championship.</p>
                  )}
                </div>
              </div>
            ...
            <div className="bg-yellow-500/10 backdrop-blur-sm border border-yellow-500/30 rounded-lg p-6 text-center">
              ...
              <p className="text-gray-300">Could not find information for the state championship event.</p>
            </div>
          )}
        </div>
      </div>
